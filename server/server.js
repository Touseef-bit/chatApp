...
async function one_to_one() {
    ...
    io.on("connection", async (socket) => {
        ...
        socket.on("sendMessage", async ({ userId, profilePicture, roomId, message }) => {
            let payload = {
                from: userId,
                profilePicture,
                roomId,
        ...
        socket.on("group-call-ask-answer", ({ to, answer }) => {
            console.log(answer)
            const target = users[to];
            console.log(`[WebRTC] Answer from ${userId} to ${to} -> socket: ${target}`);
        ...
        socket.on("ice-candidate", ({ to, candidate, roomId }) => {
            ...
            if (target) {
                ...
                io.to(target).emit("ice-candidate", { from: userId, candidate, roomId });
            }
            // to.map((el) => {
            // })
        });
